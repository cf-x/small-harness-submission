    ...
    def visit(node):
        if isinstance(node, ast.Constant) and type(node.value) in (int, float):
            ...
            if isinstance(node.op, ast.Add):
                ...
                result = left - (left / right).to_integral_value(rounding="ROUND_FLOOR") * right
...
def build_registry(store: Store, timeout=5, documents: dict | None = None):
    ...
    async def read_history(args, ctx: ToolContext):
        ...
        for row in rows:
            ...
            hits.append((score, row["seq"], {"seq": row["seq"], "role": message["role"],
                         "content": text[:1200], "truncated": len(text) > 1200}))
    ...
    registry.register(Tool("read_history", "回查当前会话已结束的历史用户输入/最终答复；压缩后细节不足时按 query 或 seq 查询。", obj({"query": {"type": "string", "maxLength": 200}, "seq": {"type": "integer", "minimum": 1}, "limit": {"type": "integer", "minimum": 1, "maximum": 3}}, []), read_history, timeout))
    return registry
