...
| 验证 | 结果 |
|---|---|
| 开发工作区离线测试 | 97 passed，4 live 测试未选中 |
| Luna 真实模型完整验收 | 4 passed，耗时 66.31 秒（本轮复测） |
| Terra 真实计算器与工具追问（前次） | 1 passed，耗时 11.57 秒 |
| 首版 ZIP 解压后新建虚拟环境、按锁文件安装、安装项目 | 成功 |
| 首版干净环境离线测试（前次） | 70 passed，4 live 测试未选中，耗时 1.93 秒 |
| 本轮 ZIP 解压后离线回归（复用现有依赖，独立源码目录） | 97 passed，4 deselected，1.70 秒；39 个文件哈希及 ZIP 完整性通过 |
...
4. 注入 200 轮确定性的测试历史，真实模型在压缩后使用 read_history 分页找回首条长消息尾部的代号；关闭并重新打开 Store/Runtime 后继续追问仍取得正确代号。
...
修复七类有复现依据的边界缺陷；同一批回归在修复前代码上为 20 failed / 77 passed，修复后为 97 passed。详见 [对抗式审查记录](ADVERSARIAL_REVIEW.md)。
